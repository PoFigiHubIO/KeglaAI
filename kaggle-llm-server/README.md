...
## Продвинутые возможности (Постоянные ссылки и Параллельный запуск)

### 1. Постоянные (статические) ссылки для туннеля
Чтобы ссылки на API/Web UI не менялись при каждом перезапуске сессии, вы можете настроить перманентный туннель в `config.yaml` в блоке `tunnel`:

* **Вариант А: Через Ngrok (бесплатно, без покупки домена)**:
  1. Зарегистрируйтесь на [ngrok.com](https://ngrok.com/) и получите один бесплатный статический домен во вкладке **Domains** (например, `my-custom-endpoint.ngrok-free.app`).
  2. Добавьте `NGROK_AUTHTOKEN` в Kaggle Secrets.
  3. В `config.yaml` укажите:
     ```yaml
     tunnel:
       provider: ngrok
       ngrok_domain: 'my-custom-endpoint.ngrok-free.app'
     ```
* **Вариант Б: Через Cloudflare Tunnel (если есть собственный домен на Cloudflare)**:
  1. Создайте Cloudflare Tunnel в панели управления Zero Trust и привяжите к нему домен (например, `kaggle.my-domain.com`).
  2. Скопируйте длинный токен туннеля (Tunnel Token) и укажите его в `config.yaml`:
     ```yaml
     tunnel:
       provider: cloudflared
       cloudflare_token: 'ВАШ_ТОКЕН_ТУННЕЛЯ'
       cloudflare_domain: 'kaggle.my-domain.com'
     ```

### 2. Параллельный запуск двух разных моделей на разных GPU (2× T4)
Вы можете полностью задействовать обе видеокарты T4 независимо друг от друга, запустив две разные модели одновременно (например, легкую модель для автодополнения кода на GPU 0 и мощную модель для чата на GPU 1).

Для этого:
1. Создайте два файла конфигурации в папке проекта, например `config_gpu0.yaml` и `config_gpu1.yaml`.
2. В `config_gpu1.yaml` измените порт (`port: 8081`) и путь к логу (`log_file: ./logs/llama-server_8081.log`), а также укажите другую модель и/или постоянную ссылку.
3. Запустите каждую модель в своей ячейке Kaggle, передав имя конфига в переменной окружения `CONFIG_FILE`:
   * **На GPU 0 (порт 8080)**: `!CONFIG_FILE=config_gpu0.yaml python start.py`
   * **На GPU 1 (порт 8081)**: `!CONFIG_FILE=config_gpu1.yaml python start.py`

Каждый инстанс автоматически оптимизируется под свой GPU, запустит свой туннель и сгенерирует изолированные VS Code конфигурации (`continue_config_8080.json` и `continue_config_8081.json`).
